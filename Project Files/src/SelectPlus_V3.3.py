"""
Select+ V3.2 - Enhanced File Manager with Advanced Features
...
- NEW: Image Processing module with resize, convert, optimize, and gallery features
- Improved media tools that show only compatible files
- Better file browsing for media operations
...
class LanguageManager:
    ...
    def __init__(self):
        # Look for settings file in config directory
        ...
        self.translations = {
            ...
            "information_misc": "📊 Information & Misc",
                ...
                "image_processing": "🖼️ Image Processing",
                "image_processing_desc": "Process and manipulate image files",
                "resize_images": "📏 Resize Images",
                "convert_images": "🔄 Convert Image Format",
                "create_image_gallery": "🖼️ Create Image Gallery",
                "optimize_images": "⚡ Optimize Images",
                "browse_compatible_files": "🗂️ Browse Compatible Files",
...
class InteractiveHandler:
    ...
    def clear_screen_fast(self):
        """Fast screen clearing using Windows API."""
        if os.name == 'nt':
            # Use faster method on Windows
            os.system('cls')
        else:
            # ANSI escape sequence for other systems
...
class MediaHandling:
    def __init__(self):
        self.audio_formats = ('.mp3', '.wav', '.flac', '.m4a', '.aac', '.ogg', '.wma')
        self.video_formats = ('.mp4', '.avi', '.mkv', '.mov', '.wmv', '.flv', '.webm')
        self.media_formats = self.audio_formats + self.video_formats
    
    def _normalize_path(self, path):
        return path.strip().strip('"')
    
    def get_compatible_media_files(self, directory_path):
        """Get all compatible media files from a directory."""
        compatible_files = []
        try:
            for file in os.listdir(directory_path):
                file_path = os.path.join(directory_path, file)
                if os.path.isfile(file_path) and file.lower().endswith(self.media_formats):
                    compatible_files.append(file_path)
        except (OSError, PermissionError) as e:
            print(f"Error accessing directory {directory_path}: {e}")
        return compatible_files
    
    def get_compatible_audio_files(self, directory_path):
        """Get all compatible audio files from a directory."""
        compatible_files = []
        try:
            for file in os.listdir(directory_path):
                file_path = os.path.join(directory_path, file)
                if os.path.isfile(file_path) and file.lower().endswith(self.audio_formats):
                    compatible_files.append(file_path)
        except (OSError, PermissionError) as e:
            print(f"Error accessing directory {directory_path}: {e}")
        return compatible_files
    
    def get_compatible_video_files(self, directory_path):
        """Get all compatible video files from a directory."""
        compatible_files = []
        try:
            for file in os.listdir(directory_path):
                file_path = os.path.join(directory_path, file)
                if os.path.isfile(file_path) and file.lower().endswith(self.video_formats):
                    compatible_files.append(file_path)
        except (OSError, PermissionError) as e:
            print(f"Error accessing directory {directory_path}: {e}")
        return compatible_files
...
class ImageProcessing:
    """Image processing functionality for resizing, converting, and optimizing images."""
    ...
    def get_compatible_files(self, directory_path):
        """Get all compatible image files from a directory."""
        compatible_files = []
        try:
            for file in os.listdir(directory_path):
                file_path = os.path.join(directory_path, file)
    ...
    def resize_images(self, image_files, width, height, output_dir=None):
        ...
        if not self.pillow_available:
            print("❌ Pillow library is required for image processing. Install with: pip install Pillow")
            return
        
        from PIL import Image
        
        if isinstance(image_files, str):
            image_files = [self._normalize_path(image_files)]
        elif isinstance(image_files, list):
            image_files = [self._normalize_path(f) for f in image_files]
        
        if not output_dir:
            output_dir = os.path.join(os.path.dirname(image_files[0]), "resized_images")
        ...
        for image_file in image_files:
            ...
            try:
                with Image.open(image_file) as img:
                    ...
                    output_filename = f"{base_name}_resized_{width}x{height}{ext}"
                    output_path = os.path.join(output_dir, output_filename)
                    
                    # Save resized image
                    resized_img.save(output_path, quality=95)
                    print(f"✅ Resized '{os.path.basename(image_file)}' to {width}x{height}")
            except Exception as e:
    ...
    def convert_images(self, image_files, target_format, output_dir=None):
        ...
        if not output_dir:
            output_dir = os.path.join(os.path.dirname(image_files[0]), "converted_images")
        ...
        for image_file in image_files:
            ...
            try:
                with Image.open(image_file) as img:
                    ...
                    print(f"✅ Converted '{os.path.basename(image_file)}' to {target_format.upper()}")
            except Exception as e:
                print(f"❌ Error converting '{os.path.basename(image_file)}': {e}")
    
    def optimize_images(self, image_files, quality=85, output_dir=None):
        """Optimize images by reducing file size while maintaining quality."""
        if not self.pillow_available:
            print("❌ Pillow library is required for image processing. Install with: pip install Pillow")
            return
        ...
        if not output_dir:
            output_dir = os.path.join(os.path.dirname(image_files[0]), "optimized_images")
        ...
        for image_file in image_files:
            ...
            try:
                with Image.open(image_file) as img:
                    ...
                    print(f"✅ Optimized '{os.path.basename(image_file)}' - {reduction:.1f}% size reduction")
            except Exception as e:
                print(f"❌ Error optimizing '{os.path.basename(image_file)}': {e}")
    
    def create_image_gallery(self, image_files, output_dir=None):
        """Create an HTML gallery from image files."""
        if isinstance(image_files, str):
            image_files = [self._normalize_path(image_files)]
        elif isinstance(image_files, list):
            image_files = [self._normalize_path(f) for f in image_files]
        ...
        if not output_dir:
            output_dir = os.path.join(os.path.dirname(image_files[0]), "image_gallery")
        
        os.makedirs(output_dir, exist_ok=True)
        
        # Filter for valid image files
        valid_images = [f for f in image_files if os.path.exists(f) and f.lower().endswith(self.supported_formats)]
        
        if not valid_images:
            print("❌ No valid image files found for gallery creation")
            return
...
<head>
    ...
    <style>
        ...
        .image-item { background: white; border-radius: 10px; padding: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
...
'''
        ...
        <div class="image-item">
            <img src="{image_name}" alt="{image_name}" onclick="window.open('{image_name}', '_blank')">
...
'''
        ...
        print(f"✅ Created image gallery with {len(valid_images)} images")
...
class SelectPlusApp:
    ...
    def load_settings(self):
        ...
        try:
            # Look for settings file in config directory
            config_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'config')
            settings_file = os.path.join(config_dir, "selectplus_settings.json")
    ...
    def _create_playlist(self):
        ...
        while True:
            input_options = [
                ...
                self.lang.get_text("browse_compatible_files"),
                self.lang.get_text("browse_directory"),
                self.lang.get_text("back")
            ]
            
            input_method = self.interactive.show_interactive_menu(
                "CREATE PLAYLIST - SELECT FILES", 
                input_options,
                context_path=os.getcwd()
            )
            ...
            if input_method == 0:  # Use selected files
                ...
                if not files_to_process:
                    ...
                    time.sleep(1)
                    continue
            elif input_method == 1:  # Browse compatible files
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.media_handler.get_compatible_media_files(directory)
                    if not files_to_process:
                        print("❌ No compatible media files found in selected directory")
                        time.sleep(1)
                        continue
                else:
                    continue
            elif input_method == 2:  # Browse directory for all media
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.media_handler.get_compatible_media_files(directory)
                    if not files_to_process:
            ...
            if files_to_process:
                ...
                self.media_handler.create_playlist(files_to_process, playlist_name)
    ...
    def _mass_extract_audio(self):
        ...
        while True:
            ...
            if input_method == 0:  # Use selected files
                ...
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.media_handler.get_compatible_audio_files(directory)
                    if not files_to_process:
                        print("❌ No compatible audio files found in selected directory")
                        time.sleep(1)
                        continue
                else:
                    continue
            elif input_method == 2:  # Browse directory for all audio
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.media_handler.get_compatible_audio_files(directory)
                    if not files_to_process:
    ...
    def _mass_trim_videos(self):
        ...
        while True:
            ...
            if input_method == 0:  # Use selected files
                ...
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.media_handler.get_compatible_video_files(directory)
                    if not files_to_process:
                        print("❌ No compatible video files found in selected directory")
                        time.sleep(1)
                        continue
                else:
                    continue
            elif input_method == 2:  # Browse directory for all video
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.media_handler.get_compatible_video_files(directory)
                    if not files_to_process:
    ...
    def _image_processing_menu(self):
        """Image processing menu with various image operations."""
        while True:
            options = [
                self.lang.get_text("resize_images"),
                self.lang.get_text("convert_images"),
                self.lang.get_text("optimize_images"),
                self.lang.get_text("create_image_gallery"),
                self.lang.get_text("back_to_main")
            ]
            ...
            if choice is None or choice == 4:  # Back or Esc
                break
            elif choice == 0:  # Resize Images
                self._resize_images()
            elif choice == 1:  # Convert Images
                self._convert_images()
            elif choice == 2:  # Optimize Images
                self._optimize_images()
            elif choice == 3:  # Create Image Gallery
    ...
    def _resize_images(self):
        ...
        while True:
            ...
            if input_method == 0:  # Use selected files
                ...
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.image_processor.get_compatible_files(directory)
                    if not files_to_process:
                        print("❌ No compatible image files found in selected directory")
                        time.sleep(1)
                        continue
                else:
                    continue
            elif input_method == 2:  # Browse directory for all images
                directory = self._browse_path(select_type="dir")
                if directory:
                    files_to_process = self.image_processor.get_compatible_files(directory)
                    if not files_to_process:
            ...
            try:
                width = int(self.interactive.show_text_input("Enter new width (pixels)"))
                height = int(self.interactive.show_text_input("Enter new height (pixels)"))
                output_dir = self.interactive.show_text_input(
                    "Enter output directory (leave empty for default)"
                ) or None
                
                self.image_processor.resize_images(files_to_process, width, height, output_dir)
                print(f"✅ Resized {len(files_to_process)} images")
                time.sleep(2)
                break
            except ValueError:
                print("❌ Invalid dimensions. Please enter numbers.")
                time.sleep(2)
    ...
    def _convert_images(self):
        ...
        while True:
            ...
            format_choice = self.interactive.show_interactive_menu(
                "SELECT TARGET FORMAT", 
                format_options,
                context_path=os.getcwd()
            )
            
            if format_choice is None or format_choice == 6:  # Back
                continue
            
            target_format = format_options[format_choice].lower()
            output_dir = self.interactive.show_text_input(
                "Enter output directory (leave empty for default)"
            ) or None
            
            self.image_processor.convert_images(files_to_process, target_format, output_dir)
            print(f"✅ Converted {len(files_to_process)} images to {target_format.upper()}")
            time.sleep(2)
            break
    ...
    def _optimize_images(self):
        ...
        while True:
            ...
            quality_choice = self.interactive.show_interactive_menu(
                "SELECT QUALITY LEVEL", 
                quality_options,
                context_path=os.getcwd()
            )
            
            if quality_choice is None or quality_choice == 5:  # Back
                continue
            
            quality_map = {0: 95, 1: 85, 2: 75, 3: 60}
            
            if quality_choice == 4:  # Custom quality
                try:
                    quality = int(self.interactive.show_text_input("Enter quality (1-100)"))
            ...
            self.image_processor.optimize_images(files_to_process, quality, output_dir)
            print(f"✅ Optimized {len(files_to_process)} images")
            time.sleep(2)
            break
    ...
    def _create_image_gallery(self):
        ...
        while True:
            ...
            input_method = self.interactive.show_interactive_menu(
                "CREATE IMAGE GALLERY - SELECT FILES", 
                input_options,
                context_path=os.getcwd()
            )
            ...
            output_dir = self.interactive.show_text_input(
                "Enter gallery output directory (leave empty for default)"
            ) or None
            
            self.image_processor.create_image_gallery(files_to_process, output_dir)
            print(f"✅ Created image gallery with {len(files_to_process)} images")
            time.sleep(2)
            break
    ...
    def run(self):
        ...
        while True:
            options = [
                ...
                self.lang.get_text("image_processing"),
            ...
            if choice is None or choice == 8:  # Exit or Esc
